...
TIMESTAMP=$(date +%s000)
QUERY_STRING="symbol=$SYMBOL&limit=$LIMIT&recvWindow=5000&timestamp=$TIMESTAMP"
SIGNATURE=$(echo -n "$QUERY_STRING" | openssl dgst -sha256 -hmac "$API_SECRET" | cut -d' ' -f2)

# Send request and format output
curl -s -H "X-MBX-APIKEY: $API_KEY" -X GET "$BASE_URL$ENDPOINT?$QUERY_STRING&signature=$SIGNATURE" | jq '[.[] | {symbol, side, price, qty, quoteQty, commission, commissionAsset, time}]'